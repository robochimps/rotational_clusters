import jax
from jax import config
from jax import numpy as jnp

config.update("jax_enable_x64", True)


@jax.jit
def poten(coords):
    """Calculates potential energy surface for the H2S molecule using data from ExoMol,
    see https://www.exomol.com/data/molecules/H2S/1H2-32S/

    Args:
        coords (ndarray of shape (npoints, 3)): An array containing the internal coordinates of the H2S molecule.
            Each row represents a set of coordinates (r1, r2, alpha), where
            - r1 (float): the S-H1 bond distance in Angstrom.
            - r2 (float): the S-H2 bond distance in Angstrom.
            - alpha (float): the valence bond angle in radians.

    Returns:
        ndarray of shape (npoints): The calculated potential energy values in cm^-1
            corresponding to each set of input coordinates.
    """
    params = [
        1.3359007e0,
        1.70400000e0,
        92.265883e0,
        0.80000000000000e06,
        0.80000000000000e05,
        0.13000000000000e02,
        0.55000000000000e01,
        0.00000000000000e00,
        0.25298724728304e01,
        0.76001446034650e01,
        0.19119869561968e05,
        -0.26337942369521e04,
        -0.34641383728936e03,
        0.37146640335080e05,
        0.10372456946123e04,
        -0.48366811961179e04,
        0.31178979423415e04,
        -0.12711176182111e04,
        -0.18726215609860e03,
        -0.11796148983875e04,
        0.47473763424544e04,
        -0.13610350468586e04,
        0.29144380025635e04,
        -0.50653357937930e04,
        -0.11097146714230e04,
        0.10375282984635e03,
        0.48047328722297e04,
        -0.14060018110572e03,
        0.22425353263397e04,
        0.17914500029660e04,
        -0.98184202293571e03,
        0.61294305063749e02,
        -0.27151696719548e04,
        0.26119013667838e04,
        -0.61703073530707e04,
        -0.48833736184025e04,
        0.10049289144454e04,
        0.28708061338852e03,
        0.67852017509792e04,
        -0.10489708631647e04,
        0.74727382498639e03,
        0.20496523413380e04,
        0.24807517966824e03,
        -0.19578362789532e04,
        0.70166633265091e02,
        0.51823676755422e04,
        -0.29736603824162e04,
        -0.39068887749886e04,
        0.22707809418082e04,
        0.23793551966211e05,
        -0.34932314882947e04,
        0.85923207392547e03,
        -0.11327987267957e05,
        0.37668571305335e03,
        -0.15953549707091e05,
        0.14890386471456e04,
        0.51484402685677e03,
        -0.54617014746870e04,
        -0.69943515992635e02,
        0.17427705522754e04,
        0.29147560120520e03,
        -0.33646004327917e04,
        -0.11202273117182e04,
        -0.38323335633188e03,
        0.26441151817245e04,
        0.19366049577246e04,
        0.38175718811604e04,
        0.30778367811357e04,
        -0.26353881747532e04,
        0.83999500575225e04,
        -0.52450951528119e04,
        -0.34729416630406e03,
        -0.50050684462558e04,
        -0.28211904320344e04,
        -0.10580696087439e05,
        0.53782996209764e04,
        -0.99734750908249e03,
        -0.74046834718425e03,
        0.00000000000000e00,
        0.00000000000000e00,
        0.00000000000000e00,
        0.00000000000000e00,
        0.00000000000000e00,
        0.00000000000000e00,
        0.00000000000000e00,
        0.00000000000000e00,
        0.00000000000000e00,
        0.00000000000000e00,
        0.00000000000000e00,
        0.00000000000000e00,
        0.00000000000000e00,
        0.00000000000000e00,
        0.00000000000000e00,
        0.00000000000000e00,
        0.00000000000000e00,
        0.00000000000000e00,
        0.00000000000000e00,
        0.00000000000000e00,
        0.00000000000000e00,
        0.00000000000000e00,
        0.00000000000000e00,
        0.00000000000000e00,
    ]

    r12, r32, alpha = coords.T
    re12 = params[0]
    aa1 = params[1]
    alphae = params[2] * jnp.pi / 180.0
    force = [0.0] + [p for p in params[3:]]

    b1 = force[1]
    b2 = force[2]
    g1 = force[3]
    g2 = force[4]

    rhh = jnp.sqrt(r12**2 + r32**2 - 2 * r12 * r32 * jnp.cos(alpha))
    vhh = b1 * jnp.exp(-g1 * rhh) + b2 * jnp.exp(-g2 * rhh**2)

    y1 = 1.0 - jnp.exp(-aa1 * (r12 - re12))
    y2 = 1.0 - jnp.exp(-aa1 * (r32 - re12))
    y3 = jnp.cos(alpha) - jnp.cos(alphae)

    v0 = force[5]

    v1 = force[6] * y3**1 + force[7] * y1**1 + force[7] * y2**1

    v2 = (
        force[8] * y3**2
        + force[9] * y1**1 * y3**1
        + force[9] * y2**1 * y3**1
        + force[10] * y1**1 * y2**1
        + force[11] * y1**2
        + force[11] * y2**2
    )

    v3 = (
        force[12] * y3**3
        + force[13] * y1**1 * y3**2
        + force[13] * y2**1 * y3**2
        + force[14] * y1**1 * y2**1 * y3**1
        + force[15] * y1**2 * y3**1
        + force[15] * y2**2 * y3**1
        + force[16] * y1**2 * y2**1
        + force[16] * y1**1 * y2**2
        + force[17] * y1**3
        + force[17] * y2**3
    )

    v4 = (
        force[18] * y3**4
        + force[19] * y1**1 * y3**3
        + force[19] * y2**1 * y3**3
        + force[20] * y1**1 * y2**1 * y3**2
        + force[21] * y1**2 * y3**2
        + force[21] * y2**2 * y3**2
        + force[22] * y1**2 * y2**1 * y3**1
        + force[22] * y1**1 * y2**2 * y3**1
        + force[23] * y1**2 * y2**2
        + force[24] * y1**3 * y3**1
        + force[24] * y2**3 * y3**1
        + force[25] * y1**3 * y2**1
        + force[25] * y1**1 * y2**3
        + force[26] * y1**4
        + force[26] * y2**4
    )

    v5 = (
        force[27] * y3**5
        + force[28] * y1**1 * y3**4
        + force[28] * y2**1 * y3**4
        + force[29] * y1**1 * y2**1 * y3**3
        + force[30] * y1**2 * y3**3
        + force[30] * y2**2 * y3**3
        + force[31] * y1**2 * y2**1 * y3**2
        + force[31] * y1**1 * y2**2 * y3**2
        + force[32] * y1**2 * y2**2 * y3**1
        + force[33] * y1**3 * y3**2
        + force[33] * y2**3 * y3**2
        + force[34] * y1**3 * y2**1 * y3**1
        + force[34] * y1**1 * y2**3 * y3**1
        + force[35] * y1**3 * y2**2
        + force[35] * y1**2 * y2**3
        + force[36] * y1**4 * y3**1
        + force[36] * y2**4 * y3**1
        + force[37] * y1**4 * y2**1
        + force[37] * y1**1 * y2**4
        + force[38] * y1**5
        + force[38] * y2**5
    )

    v6 = (
        force[39] * y3**6
        + force[40] * y1**1 * y3**5
        + force[40] * y2**1 * y3**5
        + force[41] * y1**1 * y2**1 * y3**4
        + force[42] * y1**2 * y3**4
        + force[42] * y2**2 * y3**4
        + force[43] * y1**2 * y2**1 * y3**3
        + force[43] * y1**1 * y2**2 * y3**3
        + force[44] * y1**2 * y2**2 * y3**2
        + force[45] * y1**3 * y3**3
        + force[45] * y2**3 * y3**3
        + force[46] * y1**3 * y2**1 * y3**2
        + force[46] * y1**1 * y2**3 * y3**2
        + force[47] * y1**3 * y2**2 * y3**1
        + force[47] * y1**2 * y2**3 * y3**1
        + force[48] * y1**3 * y2**3
        + force[49] * y1**4 * y3**2
        + force[49] * y2**4 * y3**2
        + force[50] * y1**4 * y2**1 * y3**1
        + force[50] * y1**1 * y2**4 * y3**1
        + force[51] * y1**4 * y2**2
        + force[51] * y1**2 * y2**4
        + force[52] * y1**5 * y3**1
        + force[52] * y2**5 * y3**1
        + force[53] * y1**5 * y2**1
        + force[53] * y1**1 * y2**5
        + force[54] * y1**6
        + force[54] * y2**6
    )

    v7 = (
        force[55] * y3**7
        + force[56] * y1**1 * y3**6
        + force[56] * y2**1 * y3**6
        + force[57] * y1**1 * y2**1 * y3**5
        + force[58] * y1**2 * y3**5
        + force[58] * y2**2 * y3**5
        + force[59] * y1**2 * y2**1 * y3**4
        + force[59] * y1**1 * y2**2 * y3**4
        + force[60] * y1**2 * y2**2 * y3**3
        + force[61] * y1**3 * y3**4
        + force[61] * y2**3 * y3**4
        + force[62] * y1**3 * y2**1 * y3**3
        + force[62] * y1**1 * y2**3 * y3**3
        + force[63] * y1**3 * y2**2 * y3**2
        + force[63] * y1**2 * y2**3 * y3**2
        + force[64] * y1**3 * y2**3 * y3**1
        + force[65] * y1**4 * y3**3
        + force[65] * y2**4 * y3**3
        + force[66] * y1**4 * y2**1 * y3**2
        + force[66] * y1**1 * y2**4 * y3**2
        + force[67] * y1**4 * y2**2 * y3**1
        + force[67] * y1**2 * y2**4 * y3**1
        + force[68] * y1**4 * y2**3
        + force[68] * y1**3 * y2**4
        + force[69] * y1**5 * y3**2
        + force[69] * y2**5 * y3**2
        + force[70] * y1**5 * y2**1 * y3**1
        + force[70] * y1**1 * y2**5 * y3**1
        + force[71] * y1**5 * y2**2
        + force[71] * y1**2 * y2**5
        + force[72] * y1**6 * y3**1
        + force[72] * y2**6 * y3**1
        + force[73] * y1**6 * y2**1
        + force[73] * y1**1 * y2**6
        + force[74] * y1**7
        + force[74] * y2**7
    )

    v8 = (
        force[75] * y3**8
        + force[76] * y1**1 * y3**7
        + force[76] * y2**1 * y3**7
        + force[77] * y1**1 * y2**1 * y3**6
        + force[78] * y1**2 * y3**6
        + force[78] * y2**2 * y3**6
        + force[79] * y1**2 * y2**1 * y3**5
        + force[79] * y1**1 * y2**2 * y3**5
        + force[80] * y1**2 * y2**2 * y3**4
        + force[81] * y1**3 * y3**5
        + force[81] * y2**3 * y3**5
        + force[82] * y1**3 * y2**1 * y3**4
        + force[82] * y1**1 * y2**3 * y3**4
        + force[83] * y1**3 * y2**2 * y3**3
        + force[83] * y1**2 * y2**3 * y3**3
        + force[84] * y1**3 * y2**3 * y3**2
        + force[85] * y1**4 * y3**4
        + force[85] * y2**4 * y3**4
        + force[86] * y1**4 * y2**1 * y3**3
        + force[86] * y1**1 * y2**4 * y3**3
        + force[87] * y1**4 * y2**2 * y3**2
        + force[87] * y1**2 * y2**4 * y3**2
        + force[88] * y1**4 * y2**3 * y3**1
        + force[88] * y1**3 * y2**4 * y3**1
        + force[89] * y1**4 * y2**4
        + force[90] * y1**5 * y3**3
        + force[90] * y2**5 * y3**3
        + force[91] * y1**5 * y2**1 * y3**2
        + force[91] * y1**1 * y2**5 * y3**2
        + force[92] * y1**5 * y2**2 * y3**1
        + force[92] * y1**2 * y2**5 * y3**1
        + force[93] * y1**5 * y2**3
        + force[93] * y1**3 * y2**5
        + force[94] * y1**6 * y3**2
        + force[94] * y2**6 * y3**2
        + force[95] * y1**6 * y2**1 * y3**1
        + force[95] * y1**1 * y2**6 * y3**1
        + force[96] * y1**6 * y2**2
        + force[96] * y1**2 * y2**6
        + force[97] * y1**7 * y3**1
        + force[97] * y2**7 * y3**1
        + force[98] * y1**7 * y2**1
        + force[98] * y1**1 * y2**7
        + force[99] * y1**8
        + force[99] * y2**8
    )

    return v0 + v1 + v2 + v3 + v4 + v5 + v6 + v7 + v8 + vhh